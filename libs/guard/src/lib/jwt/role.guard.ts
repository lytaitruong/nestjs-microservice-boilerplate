import { AppException, IReq } from '@libs/common'
...
export const Roles = (...roles: string[]) => SetMetadata(GuardType.JWT_ROLE, roles)
...
export class RoleGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> | Observable<boolean> {
    const roles = this.reflector.getAllAndOverride<string[]>(GuardType.JWT_ROLE, [
      context.getHandler(),
      context.getClass(),
    ])
