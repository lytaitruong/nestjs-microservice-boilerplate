...
import { ApiPaginateRes, ApiPassedRes, HEADERS, IReq, IRes, Pag } from '@libs/common'
import { IPaginationQueryParams } from '@libs/types'
...
import { IRestService, Str } from './rest.interface'
import { AbstractValidationPipe } from './rest.validation'

export const RestController = <R, C, U, P, Q extends IPaginationQueryParams<T>, T = any>(
  result: Type<R>,
  create: Type<C>,
  update: Type<U>,
  param: Type<P>,
  ...
  class CrudController {
    constructor(public readonly service: IRestService<R, C, U, P, Q, T>) {}
    ...
    async getAll(
      ...
      if ((result as Pag<T>).meta) {
        return res.send(result)
      }
      return res
        .header(HEADERS.CONTENT_DISPOSITION, `attachment; filename=${(result as Str).name}`)
        .send(result['data'])
    }
